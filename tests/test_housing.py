...
def test_housing_cart_create():
    ...
    c = CART(df=df_housing, y_name="price", metrics_type="regression")
    c.create_tree()
...
def test_housing_gradientboostedtree():
    ...
    gbt = GradientBoostedTree(
        df=df_housing, y_name="price", learning_rate=0.5, init_metrics_type="regression"
    )
    gbt.train(20)
...
def test_housing_randomforest():
    df_housing = pd.read_csv("data/Housing.csv")
    rf = RandomForest(df=df_housing, y_name="price", metrics_type="regression")
    rf.train(20)
