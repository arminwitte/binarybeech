#!/usr/bin/env python3
"""Compare CART on iris with scalar vs binned attribute handlers.

This script runs CART (from the package) on the iris dataset twice:
...
def bin_df_to_int(df: pd.DataFrame, n_bins: int = 5) -> pd.DataFrame:
    ...
    for col in df_b.columns:
        if pd.api.types.is_numeric_dtype(df_b[col]) and col != "species":
...
def run_cart(train_df: pd.DataFrame, test_df: pd.DataFrame, method: str = "classification") -> Tuple[float, float, float]:
    # measure training (tree creation) and prediction
    c = CART(df=train_df, y_name="species", method=method)
    ...
    acc = accuracy(test_df["species"], y_pred)
    return acc, t_fit, t_pred


def main(repeats: int = 5, n_bins: int = 5):
    df = pd.read_csv("data/iris.csv")
    results = {"scalar": [], "binned": []}

    for i in range(repeats):
        train, test = stratified_train_test_split(df, "species", test_size=0.3, seed=42 + i)

        # scalar (original floats -> IntervalAttributeHandler)
        r = run_cart(train, test, method="classification")
        ...
        r = run_cart(train_b, test_b, method="classification")
    ...
    print("CART on Iris comparison (mean over {} runs):".format(repeats))
